...
> `REMEDIATION.md` (fix-plans + patches — generated by the remediation workflow, **not yet committed**;
> lands in a follow-up PR); process in [`OPERATING-MODEL.md`](OPERATING-MODEL.md).
...
|---|---|---|---|
| core**#59** | — | atmosphere-core | **CI red**: `test-composio-sovereign.mjs` fails 1/76 on BOTH Node 20 **and** 22 (passes locally → env/network-dependent, not hermetic). Fix: make it hermetic or gate it out of CI; then reconcile Node policy. *(Not a Node-version-only issue — Codex correction.)* |
