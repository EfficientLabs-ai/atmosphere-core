...
/** Enable a messaging channel with the VAULT HANDLE(S) to its token(s) (never the token itself) + owner id. */
export function setMessagingChannel(channel, { enabled = true, tokenHandle, appTokenHandle, ownerId } = {}) {
  if (!PROVIDER_RE.test(String(channel || ''))) throw new Error('invalid channel');
  return updateConfig((c) => {
    c.messaging = { ...c.messaging };
    c.messaging[channel] = { enabled: !!enabled, tokenHandle: tokenHandle || null, appTokenHandle: appTokenHandle || null, ownerId: ownerId || null };
  });
}
